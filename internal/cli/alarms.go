...
func (alarmsListCommand) Run(runtime *Runtime) error {
	...
	alarms, err := client.ListAlarms(runtime.ctx)
	if err != nil {
		return err
	}
...
func (command alarmsCreateCommand) Run(runtime *Runtime) error {
	...
	request := &api.AlarmCreate{
		Label: command.Label,
		Schedule: api.ScheduleSchema{
			Hour: command.Hour, Minute: command.Minute,
			Timezone: api.NewOptString(command.Timezone), Days: days,
		},
	...
	if command.ProfileID != "" {
		...
		request.ProfileID = api.NewOptNilUUID(profileID)
	}
	client, err := runtime.client()
	if err != nil {
		return err
	}
	response, err := client.CreateAlarm(runtime.ctx, request)
	if err != nil {
		return err
	}
...
func (command alarmsGetCommand) Run(runtime *Runtime) error {
	...
	response, err := client.GetAlarm(runtime.ctx, api.GetAlarmParams{AlarmID: command.AlarmID})
	if err != nil {
		return err
	}
...
func (command alarmsEnableCommand) Run(runtime *Runtime) error {
	...
	response, err := client.EnableAlarm(runtime.ctx, api.EnableAlarmParams{AlarmID: command.AlarmID})
	if err != nil {
		return err
	}
...
func (command alarmsDisableCommand) Run(runtime *Runtime) error {
	...
	response, err := client.DisableAlarm(runtime.ctx, api.DisableAlarmParams{AlarmID: command.AlarmID})
	if err != nil {
		return err
	}
...
func (command alarmsSnoozeCommand) Run(runtime *Runtime) error {
	...
	response, err := client.SnoozeAlarm(runtime.ctx,
		&api.SnoozeRequest{Minutes: api.NewOptInt(command.Minutes)},
		api.SnoozeAlarmParams{AlarmID: command.AlarmID},
	)
	if err != nil {
		return err
	}
...
func (command alarmsDismissCommand) Run(runtime *Runtime) error {
	...
	response, err := client.DismissAlarm(runtime.ctx, api.DismissAlarmParams{AlarmID: command.AlarmID})
	if err != nil {
		return err
	}
...
func (command alarmsOccurrencesCommand) Run(runtime *Runtime) error {
	...
	response, err := client.ListOccurrences(runtime.ctx, api.ListOccurrencesParams{
		AlarmID: command.AlarmID, Limit: api.NewOptInt(command.Limit),
	})
	if err != nil {
		return err
	}
	switch result := response.(type) {
	case *api.ListOccurrencesOKApplicationJSON:
		occurrences := []api.OccurrenceRead(*result)
...
func (command alarmsDeleteCommand) Run(runtime *Runtime) error {
	...
	response, err := client.DeleteAlarm(runtime.ctx, api.DeleteAlarmParams{AlarmID: command.AlarmID})
	if err != nil {
		return err
	}
	switch result := response.(type) {
	case *api.DeleteAlarmNoContent:
...
func parseWeekdays(values []string) ([]api.Weekday, error) {
	days := make([]api.Weekday, 0, len(values))
	seen := map[api.Weekday]bool{}
...
func writeAlarm(runtime *Runtime, alarm *api.AlarmRead) error {
...
func alarmRow(alarm api.AlarmRead) []string {
	next := "-"
...
func formatSchedule(schedule api.ScheduleSchema) string {
...
func writeOccurrence(runtime *Runtime, occurrence *api.OccurrenceRead) error {
...
func occurrenceRow(occurrence api.OccurrenceRead) []string {
	return []string{

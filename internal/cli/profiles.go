...
func (profilesListCommand) Run(runtime *Runtime) error {
	...
	profiles, err := client.ListProfiles(runtime.ctx)
	if err != nil {
		return err
	}
...
func (command profilesCreateCommand) Run(runtime *Runtime) error {
	...
	request := &api.ProfileCreate{
		Name:  command.Name,
		Intro: api.IntroSchema{SoundID: command.IntroSoundID},
		Ringtone: api.RingtoneSchema{
			SoundID: command.RingtoneSoundID,
			Volume:  api.NewOptInt(command.RingtoneVolume),
		},
		Sunrise: api.NewOptSunriseSchema(api.SunriseSchema{
			SceneName:       api.NewOptString(command.SceneName),
			DurationMinutes: api.NewOptInt(command.DurationMinutes),
			BrightnessStart: api.NewOptInt(command.BrightnessStart),
			BrightnessEnd:   api.NewOptInt(command.BrightnessEnd),
		}),
	}
	response, err := client.CreateProfile(runtime.ctx, request)
	if err != nil {
		return err
	}
	switch result := response.(type) {
	case *api.ProfileRead:
...
func writeProfile(runtime *Runtime, profile *api.ProfileRead) error {
...
func profileRow(profile api.ProfileRead) []string {
	scene := "-"
